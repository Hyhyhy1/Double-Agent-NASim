...
class AttackerHostVector:
    ...
    3. discovered - bool
    4. value - float
    5. discovery value - float
    6. access - int
    7. OS - bool for each OS in scenario (only one OS has value of true)
    8. services running - bool for each service in scenario
    9. processes running - bool for each process in scenario

    Notes
    -----
    - The size of the vector is equal to:

        6 + #OS + #services + #processes.
    ...
    def vectorize(cls, host, vector=None):
        cls._initialize(
            host.services, host.os, host.processes
        )
    ...
    def copy(self):
        ...
        return AttackerHostVector(vector_copy)

    def numpy(self):
        return self.vector
    
    @classmethod
    def _initialize(cls, services, os_info, processes):
        cls.os_idx_map = {}
        cls.service_idx_map = {}
        cls.process_idx_map = {}
    ...
    def __repr__(self):
        return f"""Compromised: {self.compromised}, \n
Reachable: {self.reachable}, \n 
Discovered: {self.discovered}, \n
Value: {self.value}, \n
Discovery value: {self.discovery_value}, \n
Access: {self.access}, \n
OS: {self.os}, \n
Services: {self.services}, \n
Processes: {self.processes}
"""
    ...
    def __eq__(self, other):
        if self is other:
            return True
        if not isinstance(other, AttackerHostVector):
            return False
